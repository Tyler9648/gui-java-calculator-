...
public class Parser {
    ...
    public AST caseBlock() throws SyntaxError {  //case block
        expect(Tokens.LeftBrace);
        AST t = new SwitchBlockTree();         //case block

        do {
            t.addKid(caseStatement());        //case statements
        } while (startingCase());   // get decls

        if(isNextTok(Tokens.Default)){   //default statements
            scan();
            expect(Tokens.HashDelimeter);
            expect(Tokens.Colon);
            AST t1 = new DefaultTree();
    ...
    public AST caseStatement() throws SyntaxError {  //case block
        AST t = new CaseTree();         //each case
        scan();
        ...
        t.addKid(rFactor());            //add case number(s)
        } while (isNextTok(Tokens.Comma));

        expect(Tokens.RightBracket);    // ]
        expect(Tokens.HashDelimeter);   // #
        expect(Tokens.Colon);           // :
        t.addKid(rStatement());         //statement should be right after :
        return t;
    }

    boolean startingCase() {     //if next token is a case
    ...
    public AST rStatement() throws SyntaxError {
        ...
        if (isNextTok(Tokens.Switch)) {
            scan();
            t = new SwitchTree();       //Top of switch tree
            expect(Tokens.LeftParen);
            t.addKid(rName());          //<id> inside of switch()
            expect(Tokens.RightParen);
            t.addKid(caseBlock());
            return t;
    ...
    public AST rFactor() throws SyntaxError {
        ...
        if (isNextTok(Tokens.Utf16StringLit)) {
            t = new Utf16StringLitTree(currentToken);
            scan();
            return t;
        }
